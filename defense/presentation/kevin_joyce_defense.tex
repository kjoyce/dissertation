...
\author{Kevin Joyce$^{\dagger*}$\\ {\small advised by John Bardsley$^\dagger$ and Aaron Luttman$^{*}$}} %, \texttt{kevin1.joyce@umt.edu}}
...
\begin{frame}
  ...
  \includegraphics[width=.5\textwidth]{figures/cygnus_image_1.jpg}
  \includegraphics[width=.5\textwidth]{figures/nnss_cygnus_0.jpg} 
...
\begin{frame}[t]
  ...
  \begin{itemize}
    \itemsep 1.2em
    \item When $k(x,y) = p(\sqrt{x^2+y^2}) = p(T(x,y))$ is a function, then
  \begin{align*}
...
%    &= \sum_{ij}\iint_{R} \rho(r)\cdot  \phi \circ T_{ij}^{-1}(r,t)\left|dT_{ij}\right|drdt \\
    &= \int_0^\infty \rho(r) \left(\int_0^{\sqrt{r}} \sum_{ij}\phi \circ T_{ij}^{-1}(r,t)\left|dT_{ij}\right|dt \right)dr.
  \end{align*}
%    \item So we define $T_\sharp \DD(\Omega_2) \to \DD(\Omega_1)$ by
%  \begin{align*}
%    [T_\sharp\phi](r) 
%      &= \sum_{ij}\iint\limits_{T_{ij}(\Omega_{ij})} \phi \circ T_{ij}^{-1}(r,t)\left|dT_{ij}\right|dt \\
%      &= \frac{r^{\frac 1a -1} }{2a}\sum_{ij}\int_0^{r^{\frac{1}{2a}}} \phi \left((-1)^i\sqrt{r^{\frac 1a} - t^2}, (-1)^jt\right) \left( r^{\frac 1a} - t^2 \right)^{-1/2} dt
%  \end{align*}
\begin{tikzpicture}[scale=.8]
  \draw[dotted,thick] (0,-2.2) -- (0,2.2);
  \draw[dotted,thick] (-3,0) -- (3,0);
...
%        &= \int_0^\infty p \cdot \alert{\underbrace{\left(-\frac 1r \frac{d}{dr}r \frac{d}{dr}\right)}_{\mathcal L} }p(r) rdr
%    \end{align*}
...
\begin{flalign*}
1.&\text{ Simulate }\lambda^{k+1}\sim \Gamma\left((2N+1)/2+\alpha,\frac{1}{2}\Vert\vect G\vect p^{k}-\vect b\Vert^2+\beta\right).&\\
2.&\text{ Set } \lambda = \lambda^{k+1},\delta = \delta^k \text{ and compute }\alert{\vect R_{\lambda,\delta}\, \vect m_{\lambda,\delta},\text{ then } c(\vect R,\lambda,\delta).}\\
  &\text{ For }j=1\dots n_{mh}\\
  &\quad\text{i.~Simulate }w \sim \N(0,1)\text{ and set }\delta' = \exp(\gamma w + \delta)\\
  &\quad\text{ii.~Compute }\alert{\vect R_{\lambda,\delta'}, \vect m_{\lambda,\delta'},\text{ then } c(\vect R,\lambda,\delta')}.\\
  &\quad\text{iii.~Simulate }u\sim U([0,1])\text{ and }\\
  &\quad\quad\text{if }\ln u > \min\left\{0,c(\vect R,\lambda,\delta')-c(\vect R_{\lambda,\delta},\vect m_{\lambda,\delta},\delta)\right\}\\
...
\begin{frame}[t]
  ...
  \begin{itemize}
    ...
    \includegraphics[width=.75\textwidth]{figures/ESS_vs_substeps.pdf}
