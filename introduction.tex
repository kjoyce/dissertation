...
\subsection{Organization}
  ...
  \Cref{chapter:computational} will give details on how to carry out the estimation on a computer.
  ...
  Finally, in \Cref{chapter:results}, we present the results of an implementation on synthetically derived data and on measured data from a high-energy X-ray imaging system at the U.S.~Department of Energy's Nevada National Security Site.
...
\section{Modeling blur with a PSF}
  ...
  \textcolor{red}{say what $b$ is in this sentence}
...
\end{equation}
  ...
  That is, if the calibration object completely attenuates X-rays along a vertical edge at the fixed location at $s=0$ in the imaging plane, then $E=\{(s,t):s\ge0\}$ and $f(s,t) \eqdef f_E(s) = 1$ if $s\ge0$ and $0$ if $s <0$; see \Cref{fig:edgePicture,fig:edgeData} for a schematic of the calibration object and measurement system and an example of recorded intensity data.
...
\end{equation} 
  ...
  Solving the integral equation in \eqref{eq:psfForwardModelDeterministic} will be the primary focus of this work.
