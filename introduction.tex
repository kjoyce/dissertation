...
Each of these different mathematical formulations provide insight into the point spread estimation problem, e.g.~one formulation shows that a blurred edge is sufficient for estimation, and another is useful for seeing that the problem is ill-posed.  
  ...
  We take a Bayesian approach to estimation and uncertainty quantification, and \Cref{chapter:mcmcTheory} will lay out prerequisite Markov Chain theory and general algorithms used in this work.
  ...
  \Cref{chapter:computational} then develops these notions needed specifically to carry out the estimation on a computer, and carries out the program of estimation and uncertainty quantification on synthetically derived and on real radiographic data.
...
\end{equation} 
  ...
  \end{equation}
  for all $s$ since the integrand is odd in $t$, and given any solution $k$, $k(s,t)+te^{-t^2 - s^2}$.
...
\end{figure}
  ...
  Defining these spaces rigorously is the main focus of \Cref{chapter:theoretical} and it will be shown that both of these spaces separable Hilbert spaces, and that the data are a subspace of $L^2((-\infty,0])$.
